...
export interface GalleryData {
  title: string;
  particles: { name: string; game: string; sprite?: string; spec: ParticleSpec }[];
  models: { name: string; game: string; glb: string }[];
  sounds: { name: string; game: string; src: string; fmt: string; dur: string }[];
  textures: { name: string; game: string; src: string }[];
}
...
export function buildGalleryHtml(d: GalleryData): string {
  ...
  const particleCards = d.particles
    .map(
      (p) => `<div class="card"><canvas class="particle" width="260" height="220"
  ...
  const modelCards = d.models
    .map(
      (m) => `<div class="card"><model-viewer src="${esc(m.glb)}" camera-controls auto-rotate disable-zoom
  ...
  const soundCards = d.sounds
    .map(
      (s) => `<div class="card snd"><div class="nm" title="${esc(s.name)}">${esc(s.name)}</div>
  ...
  const textureCards = d.textures
    .map(
      (t) => `<div class="card"><div class="media"><img loading="lazy" src="${esc(t.src)}"></div>
...
<style>
  ...
  .card{background:#121724;border:1px solid #222b3d;border-radius:10px;overflow:hidden}
...
<h1>${esc(d.title)}</h1>
<p class="lead">Decoded out-of-engine via ValveResourceFormat — no Dota launch. Particles are replayed from their real .vpcf parameters as additive billboards (an approximation of the effect, not the engine renderer); models are interactive 3D; sounds have a player.</p>
